...
export const useLeaderboard = () => {
  ...
  const { global, regional, weekly, scope, region, loading, error, lastUpdated } = useSelector(
    (state: RootState) => state.leaderboard
  );

  const fetchLeaderboard = useCallback(
    async (selectedScope: 'global' | 'regional' | 'weekly' = 'global', selectedRegion?: string, limit: number = 50) => {
  ...
  const changeScopeAction = useCallback(
    (newScope: 'global' | 'regional' | 'weekly') => {
      dispatch(setScope(newScope));
    },
    [dispatch]
  );

  const changeRegionAction = useCallback(
    (newRegion: string) => {
